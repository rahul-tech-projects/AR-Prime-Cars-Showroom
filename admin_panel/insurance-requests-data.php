<?php
    ...
    $car_model_query = "SELECT DISTINCT car_model FROM insurance_form";
    ...
    // Build the SQL query
    $query = "SELECT id, name, email, mobile, city, car_model, carRegisterNo, 
                    car_manuf_year, insurance_type, insurance_option, insuranceCompany, 
                    insurance_start_date, insurance_end_date, comments, created_at 
            FROM insurance_form 
            WHERE 1";
    ...
    // Add date filters
    if (!empty($filter_date) && !empty($date_filter_type)) {
    ...
    // Add start and end date range filter
    if (!empty($start_date)) {
        $query .= " AND DATE(created_at) >= ?";
        $params[] = $start_date;
        $types .= 's';
    ...
    // Status message
    $status = ($result && mysqli_num_rows($result) > 0) ? "" : "No insurance requests found!";
...
<head>
    ...
    <title>Insurance Requests | AR Admin</title>
...
<div class="table-container">
    ...
    <h2 class="FormName">Insurance Requests</h2>

    <form method="GET" action="insurance-requests-data.php" class="filter-container">
        <div>
        ...
        <div>
            ...
            <button type="button" class="clear-btn" onclick="window.location.href='insurance-requests-data'">Clear Filters</button>
        </div>
    </form>
        ...
        <tbody>
            ...
            if ($result && mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<td>" . htmlspecialchars($row['car_manuf_year']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['insurance_option']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['insurance_type']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['insuranceCompany']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['insurance_start_date']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['insurance_end_date']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['comments']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['created_at']) . "</td>";
                    echo "<td><a href='delete.php?id=" . $row['id'] . "&page=insurance' class='delete-btn' onclick='return confirm(\"Are you sure you want to delete?\");'>Delete</a></td>";
                    echo "</tr>";
                }
            }
            ?>
